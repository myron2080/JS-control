...
if (jQuery) (function($) {
	...
	function _build(data) {
		...
		if (data.controls.zoomIn || data.controls.zoomOut || data.controls.next || data.controls.previous) {
			...
			html += '<span class="zoomer-previous"></span>';
			html += '<span class="zoomer-editor"></span>';
			html += '<span class="zoomer-zoom-rLeft"></span>';
			html += '<span class="zoomer-zoom-rRight"></span>';
			html += '<span class="zoomer-zoom-out"></span>';
			html += '<span class="zoomer-zoom-in"></span>';
			html += '<span class="zoomer-next"></span>';
			html += '</div>';
			...
			data.controls.$rRight = data.$zoomer.find(".zoomer-zoom-rRight");//右旋转
			data.controls.$rLeft = data.$zoomer.find(".zoomer-zoom-rLeft");//右旋转
		...
		data.controls.$rRight.on("click.zoomer", data, _rRight);
		data.controls.$rLeft.on("click.zoomer", data, _rLeft);
	...
	var _rAngle=0;
	function _rRight(data){
		_rAngle+=90;
		//data.data.$image.rotate({animateTo:_rAngle});
		data.data.$image.rotate(_rAngle);
	}
	function _rLeft(data){
		_rAngle-=90;
		data.data.$image.rotate({animateTo:_rAngle});
	}
	...
	function _onImageLoad(e) {
		...
		data.$holder.css({
		 ...
		 /*  if(data.source){
			     var ctx = data.$holder[0].getContext('2d');
		         ctx.drawImage(data.$image[0],0,0);
		  } */
