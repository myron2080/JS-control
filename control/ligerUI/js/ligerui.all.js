...
(function ($)
{
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            {
            	if(typeof(p.callBackFun) =='function'){
            		p.callBackFun();
            	}
                if (p.isHidden)
                    g.hide();
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        _fillGridBody: function (data, frozen)
        {
            ...
            //行高自适应,需要处理行序列号
            if(!p.fixedCellHeight && p.rownumbers){
            	if(g.rows && g.rows.length > 0){
            		for(var i = 0; i < g.rows.length; i++){
            			$(document.getElementById('tableContainer|1|'+g.rows[i]['__id'])).height($(document.getElementById('tableContainer|2|'+g.rows[i]['__id'])).height());
            		}
            	}
            }
            $("> div:first", g.gridbody).width(g.gridtablewidth);
        ...
        _setEvent: function ()
        {
            ...
            $(document).bind("keyup.grid", function (e)
            {
                delete g.ctrlKey;
                if(e.keyCode==13){
                	if(g.editor && g.editor.editing){
                		var c = g._getNextEditCell(g.editor);
                    	if(c){
                			$(c).click();
                			g.editor.input.focus();
                    	}
                    }
                }
            });
        ...
        _getNextEditCell:function(editor){
        	if(editor && editor.editing){
        		var g = this, p = this.options;
        		//var editParm = { record: rowdata, value: currentdata, column: column, rowindex: rowindex };
        		var editParam = editor.editParm;
        		//var rowid = rowdata['__id'];
                //var rowindex = rowdata['__index'];
        		var ce;
        		var columns = g.getColumns();
        		for(var i = 0; i < columns.length; i++){
        			if(columns[i]['__id'] == editParam.column['__id']){
        				if(i < columns.length - 1){
        					if(columns[i+1].editor){
        						ce = g.getCellObj(editParam.record, columns[i+1]);
        						break;
        					}
        				}
        			}
        		...
        		if(ce==null){
        			var rowindex = editParam.record['__index'];
        			if(rowindex < g.getData().length - 1){
        				var v = g.getRowObj(rowindex + 1);
        				for(var i = 0; i < columns.length; i++){
        					if(columns[i].editor){
        						ce = g.getCellObj(v, columns[i]);
        						break;
        					}
                		}
        			}
    ...
    $.ligerui.controls.Layout.ligerExtend($.ligerui.core.UIComponent, {
        ...
        { 
            ...
            if (g.center)
            {
                ...
                g.center.width(g.centerWidth - 5);
...
(function ($)
{
    ...
    $.ligerui.controls.Menu.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addItem: function (item, menu)
        {
            ...
            item.icon && ditem.prepend('<div class="l-menu-item-icon l-icon-' + item.icon + '" style="background:url(\''+item.icon+'\');"></div>');
...
(function ($)
{
    ...
    $.ligerui.controls.ToolBar.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addItem: function (item)
        {
            ...
            var ditem =  ditem = $('<div title="'+item.text+'" class="l-toolbar-item l-panel-btn"><span></span><div class="l-panel-btn-l"></div><div class="l-panel-btn-r"></div></div>');
            /*if(item.icon=='add' || item.icon=='ADD'){
            	ditem.css("background","#FA8919");
            }else if(item.icon=='search' || item.icon=='SEARCH'){
            	ditem.css("background","#50B4E6");
            }else if(item.icon=='modify' || item.icon=='MODIFY'){
            	ditem.css("background","#79BF56");
            }*/
            
           if(item.icon=='locate'){
        	   g.toolBar.append($('<input id="'+item.id+'" style="float:left;margin-left:2px;width:110px;"/>'));
        	   inputEnterSearch(item.id,item.click);
           } 
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                var o = data[i];
                if (o.isexpand&& o.isexpand == "true") isExpandCurrent = true;
