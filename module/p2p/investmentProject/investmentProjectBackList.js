...
$(document).ready(function(){
	...
	$list_dataGrid = $("#tableContainer").ligerGrid($.extend($list_defaultGridParam,{
        columns: [ 
			...
			{display: '一级审批', name: 'oneBackperson.name', align: 'center', width: '5%',render:app1},
			{display: '二级审批', name: 'twoBackperson.name', align: 'center', width: '5%',render:app2},
		  ],	/*,
			{display: '操作', name: 'operate', align: 'center', width: '12%',render:operateRender}*/
      
        url:getPath()+"/p2p/investmentProject/listDate?sourf2=1",
...
function ViewProject (obj){
	var flag = false;
	var dlg = art.dialog.open(getPath()+"/p2p/investmentProject/view?id="+obj.id+"&sourceFrom=BACK", {
		init : function() {
		},
		id : 'showGardenWindow',
		width : 980,
...
function projectView(pid){	
	var flag = false;
	var dlg = art.dialog.open(getPath()+"/p2p/investmentProject/view?id="+pid+"&sourceFrom=BACK", {
		init : function() {
		},
		id : 'showGardenWindow',
		width : 980,
		...
		button : [ {
			...
			name : '确定',
			 callback: function () {
				 flag = true;
			 	if(dlg.iframe.contentWindow && dlg.iframe.contentWindow.toBack){
			 		dlg.iframe.contentWindow.toBack(dlg);
				}
				return false;
			 }
		} ],
...
function selectList(){
	$list_dataParam['backstatus'] =$(".hover").attr("id");
	//录入时间
	var startDate = "";
	var endDate = "";
...
function operateRender(data,filterData){
	...
	if(data.status=='RECRUITMENT'){
		returnStr+='<a href="javascript:projectView(\''+data.id+'\');">查看</a> | <a href="javascript:publishProject(\''+data.id+'\',\'UNPUBLISHED\');">取消发布</a> | <a href="javascript:publishProject(\''+data.id+'\',\'FLOWSTANDARD\');">确定流标</a>';
	}
