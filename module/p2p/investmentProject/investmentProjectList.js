...
$(document).ready(function(){
	...
	$list_dataGrid = $("#tableContainer").ligerGrid($.extend($list_defaultGridParam,{
        columns: [ 
			...
			/*{display: '开始日期', name: '', align: 'center', width: '8%',hide:true},
			{display: '截止日期', name: '', align: 'center', width: '8%',hide:true},
			{display: '需还款(期)', name: '', align: 'center', width: '8%',hide:true},
			{display: '已还款(期)', name: '', align: 'center', width: '8%',hide:true},
			{display: '按时还款(期)', name: '', align: 'center', width: '8%',hide:true},*/
			{display: '流标原因', name: 'backRmmark', align: 'center', width: '5%',hide:true},
			{display: '操作', name: 'operate', align: 'center', width: '12%',render:operateRender}
        ],
        url:getPath()+"/p2p/investmentProject/listDate",
...
function okflow(id,status){
	...
	var dlg = art.dialog({
	    ...
	    button : [ {
			...
			name : '确定',
			 callback: function () {
				 $.post(getPath()+"/p2p/investmentProject/publishProject",{id:id,status:status,remark:$("#flowremark").val()},function(data){
						if(data.SUCESS){
							art.dialog.tips(data.SUCESS,null,"succeed");
							art.dialog.list["okflow"].close();
						}else{
...
function ViewProject(obj){
	art.dialog.open(getPath()+"/p2p/investmentProject/view?id="+obj.id+"&sourceFrom=SPV", {
		init : function() {
		},
		id : 'showGardenWindow',
		width : 980,
...
function operateRender(data,filterData){
	var returnStr = "";
	if(data.status=='PUBICSHAPP'){
		 returnStr+='<a href="javascript:projectView(\''+data.id+'\');">预览</a> 发标审批中';
	}
	if(data.status=='FLOWAPP'){
		 returnStr+='<a href="javascript:projectView(\''+data.id+'\');">预览</a> 流标审批中';
	}
	if(data.status=='FULLSCALEAPP'){
		 returnStr+='<a href="javascript:projectView(\''+data.id+'\');">预览</a> 借款确认中';
	}
	if(data.status=='UNPUBLISHED'){
		 returnStr+='<a href="javascript:projectView(\''+data.id+'\');">预览</a> |  <a href="javascript:updateProject(\''+data.id+'\');">修改</a> | <a href="javascript:publishProject(\''+data.id+'\',\'PUBICSHAPP\');">发布</a>';
	}
	if(data.status=='RECRUITMENT'){
		returnStr+='<a href="javascript:projectView1(\''+data.id+'\');">查看</a> |  <a href="javascript:okflow(\''+data.id+'\',\'FLOWAPP\');">确定流标</a>';
	}
	if(data.status=='FULLSCALE'){
		returnStr+='<a href="javascript:projectView1(\''+data.id+'\');">查看</a> |<a href="javascript:publishProject(\''+data.id+'\',\'FULLSCALEAPP\');">确定借款</a>';
	}
	if(data.status=='REPAYMENT'){
		returnStr+='<a href="javascript:projectView1(\''+data.id+'\');">查看</a>';
	}
	if(data.status=='OVER'){
		returnStr+='<a href="javascript:projectView1(\''+data.id+'\');">查看</a>';
	}
...
function publishProject(pid,status){
	if(status=='REPAYMENT'){
		art.dialog.confirm("确定借款？",function(){
...
function projectView1(pid){	
	art.dialog.open(getPath()+"/p2p/investmentProject/view?id="+pid+"&sourceFrom=v", {
		init : function() {
		},
		id : 'showGardenWindow',
		width : 980,
...
function projectView(pid){	
	art.dialog.open(getPath()+"/p2p/investmentProject/view?id="+pid+"&sourceFrom=SPV", {
		init : function() {
		},
		id : 'showGardenWindow',
		width : 980,
