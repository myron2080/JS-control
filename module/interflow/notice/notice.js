		...
		if(pag!=null){
			...
			$('<li>'+
                    '<div class="h_gg">'+
			             ...
			             '<div class="hot_r">'+
			                   '<dl>'+
			                   (((hasfbgg=='true'||hasfbgg==true)&&(data[i].author.id==currentId||usertype=='T01'))?('<dt><a onclick=modify("'+data[i].id+'") href="javascript:void(0)">修改</a></dt>'+
...
function sendWxMessage(id){
	art.dialog.confirm("是否确定发送？",function(){
		$.post(getPath()+"/interflow/notice/sendWxMessage",{id:id},function(res){
			if(res.STATE=="SUCCESS"){
	...
	var dlg = art.dialog.open(getPath()+"/interflow/notice/addNotice",{
			title:'发布公告',
			 ...
			 button:[{name:'保存',callback:function(){
					...
					if(dlg.iframe.contentWindow && dlg.iframe.contentWindow.valiNoticeSendForm){
						dlg.iframe.contentWindow.valiNoticeSendForm(dlg);
					}
					return false;
...
function managerNotice(){
	var h = $(window).height() ; 
	var w = $(window).width() ;
	var dlg = art.dialog.open(getPath()+"/interflow/companyTrend/companyFirm",{
		title:'管理公告',
		 lock:true,
		 width:(w - 200 )+ 'px',
		 height:(h - 100) + 'px',
