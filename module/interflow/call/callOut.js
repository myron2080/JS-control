...
var CallOut = {
	...
	show:function(name,phone,objectId){
		...
		var openUrl = ctx + "/interflow/call/index" ;//话务中心后期版
		...
		callCenter=$.ligerDialog.open({
			...
			buttons:[
		        {text:'关闭',onclick: function (item, dialog){ 
		        	if(dialog.jiframe && dialog.jiframe[0].contentWindow && dialog.jiframe[0].contentWindow.CallCenter.winClose){
		        		dialog.jiframe[0].contentWindow.CallCenter.winClose(dialog);
					}
		        }}
			]});
	},
	...
	showInter:function(){
		...
		var openUrl = getPath()+"/interflow/call/index" ;//话务中心后期版
		openUrl += "?t="+(new Date()).getTime(); ;
		callCenter=$.ligerDialog.open({
			width:425,
			height:240,
