$(document).ready(function(){
	...
	$list_dataGrid = $("#tableContainer").ligerGrid($.extend($list_defaultGridParam,{
        columns: [ 
            ...
            {display: '业务类型', name: 'busType', align: 'left', width: 230},
        ...
        width:"98%",
        url:getPath()+'/interflow/callCostQuery/listData',
        delayLoad:true,
        usePager:true,
        enabledSort:false
...
function syncCallList(){
	...
	$.post(getPath()+"/interflow/callCostQuery/syncCallList",{},function(data){
...
function getLastSyncTime(){
	$.post(getPath()+"/interflow/callCostQuery/syncLastTime",{},function(data){
...
function syncCallListDay(){
	...
	$.post(getPath()+"/interflow/callCostQuery/syncCallListDay",{syncTime:syncTime},function(data){
		if(data.STATE == 'SUCCESS'){
			art.dialog.tips("成功同步 "+data.syncCount+" 条话单记录！");
