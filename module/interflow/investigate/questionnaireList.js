...
function bindEvent(){
	...
	$("#searchBtn").bind("click",function(){queryQuestionnaire();});	
...
function addRow(){ 
	var $list_addUrl =  getPath()+"/interflow/questionnaire/add";
	var flag = true;
...
function editRow(rowData){ 
	var $list_addUrl =  getPath()+"/interflow/questionnaire/edit?id="+rowData.id;
	var flag = true;
	var dlg = art.dialog.open($list_addUrl,
			{title:"修改问卷",
			 lock:true,
...
function viewRow(rowData){
	var viewUrl =  getPath()+"/interflow/questionnaire/toView?id="+rowData.id;
	//可个性化实现
	if(viewUrl && viewUrl!=''){
...
function viewUnFinish(id,type){
	var dataUrl="/interflow/questionnaire/unFinishData"
	var viewUrl =  getPath()+"/interflow/questionnaire/unFinish?id="+id+"&type="+type+"&url="+dataUrl;
	//可个性化实现
	if(viewUrl && viewUrl!=''){
...
function deleteRow(rowData){
	var $list_deleteUrl =  getPath()+"/interflow/questionnaire/delete";
		...
		art.dialog.confirm('确定删除该行数据?',function(){
			$.post($list_deleteUrl,{id:rowData.id},function(res){
				...
				if(res.STATE=='SUCCESS'){
					if(typeof(queryQuestionnaire)=='function'){
						queryQuestionnaire();
					}
					refresh();
				}
...
function queryQuestionnaire(page){
	...
	$.post(ctx+"/interflow/questionnaire/listData",param,function(res){
...
function publish(id){
	...
	$.post(getPath()+"/interflow/questionnaire/updateState",{id:id},function(data){
