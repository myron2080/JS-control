...
{
	/*var monthData = manager.getData();
	for(rowObjIndex in monthData){
		if(monthData[rowObjIndex].personId!=row.personId){
		  manager.collapseDetail(rowObjIndex);
		}
	}*/
	//manager.extendDetail(row);
    ...
    var ligerGridObj =  $(grid).css('margin',10).ligerGrid($.extend($list_defaultGridParam,{
        columns:[
	            ...
	            {display: '备注', name: 'remark', align: 'left', width: 115,resizable:false},
	            {display: '操作', name: 'operate', align: 'center', width: 44,render:operateRender,resizable:false}
                ], 
...
function afterShowData(data){
	...
	//$(".l-grid-header-table").get(1).width = $(".l-grid-detailpanel-inner").get(0).clientWidth;
	//$(".l-grid-header-table").get(1).style.width = $(".l-grid-detailpanel-inner").get(0).clientWidth;
}
function bindEvent(){
	...
	$("#toAddBtn").bind("click",function(){
		var url = getPath()+'/hr/attendance/toEntry2';
		window.top.addTabItem("entryAttendance",url,"考勤录入");
	});
	
	//提交按钮
	$("#submitBtn").bind("click",function(){
		updateAttendanceState('SUBMIT');
	});
	//导出按钮
