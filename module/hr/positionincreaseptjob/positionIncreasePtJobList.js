$list_editUrl = getPath()+"/hr/positionincreaseptjob/edit";//编辑及查看url
$list_addUrl = getPath()+"/hr/positionincreaseptjob/add";//新增及查看url
$list_deleteUrl = getPath()+"/hr/positionincreaseptjob/delete";//删除url
$list_dataType = "员工兼职";
$list_editWidth =750;
$list_editHeight = 350;
var manager, g;
$(document).ready(function(){
	...
	g =  manager = $list_dataGrid = $("#tableContainer").ligerGrid($.extend($list_defaultGridParam,{
        ...
        columns: [ 
            ...
            {display: '兼职部门', name: 'applyChangeOrg.name', align: 'left', width: 120},
            {display: '兼职职位', name: 'applyChangePosition.name', align: 'left', width: 120},
            {display: '兼职职级', name: 'applyChangeJoblevel.name', align: 'left', width: 120},
        ...
        delayLoad:true,
        url:getPath()+'/hr/positionincreaseptjob/listData'
...
function approveSingle(approveType,billId){
	...
	art.dialog.confirm("是否确认审批所选申请单？",function(){
		$.post(getPath()+"/hr/positionincreaseptjob/approve",{billIds:billId,approveType:approveType},function(data){
			if(data.counter > 0){
...
function backApprove(approveType,billId){
	...
	art.dialog.confirm("反审核可能会影响业务数据,是否确认执行反审核？",function(){ 
		$.post(getPath()+"/hr/positionincreaseptjob/backApprove",{billId:billId,approveType:approveType},function(data){
...
function approve(approveType){
	...
	art.dialog.confirm("是否确认"+tipTitle+"所选申请单？",function(){
		$.post(getPath()+"/hr/positionincreaseptjob/approve",{billIds:billIds,approveType:approveType},function(data){
			if(data.counter > 0){
				art.dialog.data("MSG",null);
				resetList();
			} else {
...
function searchData(){
	...
	$list_dataParam['changeType'] = "INCREASE_PARTTIMEJOB";
