$list_editUrl = getPath()+"/hr/employeedelrundisk/edit";//编辑及查看url
$list_addUrl = getPath()+"/hr/employeedelrundisk/add";//新增及查看url
$list_deleteUrl = getPath()+"/hr/employeedelrundisk/delete";//删除url
$list_dataType = "跑盘员删除";
$list_editWidth =750;
$list_editHeight = 350;
var manager, g;
$(document).ready(function(){
	...
	g =  manager = $list_dataGrid = $("#tableContainer").ligerGrid($.extend($list_defaultGridParam,{
        ...
        columns: [ 
            ...
            {display: '职位', name: 'applyPosition.name', align: 'left', width: 120},
            {display: '职级', name: 'applyJoblevel.name', align: 'left', width: 120},
        ...
        delayLoad:true,
        //height: '95%',
        url:getPath()+'/hr/employeedelrundisk/listData'
...
function operateRender(data,filterData){
	...
	if(data.billStatus == 'SUBMIT'&&$("#jump").val()=='true') { 
		if(operateStr){
			operateStr += " |";
		} 
...
function approve(approveType){
	...
	art.dialog.confirm("是否确认"+tipTitle+"所选申请单？",function(){
		$.post(getPath()+"/hr/employeedelrundisk/approve",{billIds:billIds,approveType:approveType},function(data){
			if(data.counter > 0){
...
function approveSingle(approveType,billId){
	...
	art.dialog.confirm("是否确认审批所选申请单？",function(){
		$.post(getPath()+"/hr/employeedelrundisk/approve",{billIds:billId,approveType:approveType},function(data){
			if(data.counter > 0){
				art.dialog.data("MSG",null);
				resetList();
			} else {
...
function searchData(){
	...
	$list_dataParam['changeType'] = "DELRUNDISK";
