 ...
 * @since 2012-04-02
 * */
$list_addUrl = getPath()+"/hr/affair/leaveOffice/add";//新增url
$list_editUrl = getPath()+"/hr/affair/leaveOffice/edit";//编辑及查看url
$list_deleteUrl = getPath()+"/hr/affair/leaveOffice/delete";//删除url
$list_editWidth = "952px";
$list_editHeight = "335px";
$list_dataType = "员工离职";//数据名称
...
$(document).ready(function(){
	...
	$list_dataGrid = $("#tableContainer").ligerGrid($.extend($list_defaultGridParam,{
        ...
        checkbox:true,
        delayLoad:true,
        url:getPath()+'/hr/affair/leaveOffice/listData'
    }));
	params ={};
...
function batchApp(){
	...
	art.dialog.confirm("你选择了"+housePowers.length+"个可审批的单据,是否全部通过审批？",function(){
		...
		$(array).each(function(){
			...
			var billId = this.id;
			$.post(ctx+"/hr/affair/leaveOffice/approvalBill",{billId:billId},function(data){
					if(data.STATE=="SUCCESS"){
						true_count+=1;
...
function changeStatu(id,billStatu,opName){
	art.dialog.confirm("是否"+opName+"此单据？",function(){
		$.post(ctx+"/hr/affair/leaveOffice/changeStatu",{billId:id,billStatu:billStatu},function(data){
    		if(data.STATE=="SUCCESS"){
    			art.dialog.tips(data.MSG);
    			resetList();
...
function approval(id){
	art.dialog.confirm("是否通过审批？",function(){
		$.post(ctx+"/hr/affair/leaveOffice/approvalBill",{billId:id},function(data){
    		if(data.STATE=="SUCCESS"){
    			art.dialog.tips(data.MSG);
    			resetList();
    			art.dialog.list["appView"].close();
...
function viewRow(rowData){
	...
	if($list_editUrl && $list_editUrl!=''){
		...
		var dlg = art.dialog.open($list_editUrl+paramStr,
				...
				button:[{name:'审核',callback:function(){
