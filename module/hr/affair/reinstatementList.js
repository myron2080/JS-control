 ...
 * @since 2012-04-02
 * */
$list_addUrl = getPath()+"/hr/affair/reinstatement/add";//新增url
$list_editUrl = getPath()+"/hr/affair/reinstatement/edit";//编辑及查看url
$list_deleteUrl = getPath()+"/hr/affair/reinstatement/delete";//删除url
$list_editWidth = "952px";
$list_editHeight = "335px";
$list_dataType = "员工复职";//数据名称
...
$(document).ready(function(){
	...
	$list_dataGrid = $("#tableContainer").ligerGrid($.extend($list_defaultGridParam,{
        columns: [ 
	        {display: '操作', name: 'option', align: 'center', width: 65},
			{display: '单据编号', name: 'number', align: 'left', width: 120},
			{display: '单据状态', name: 'billStatuName', align: 'center', width: 65},
        ...
        delayLoad:true,
        //height: '95%',
        url:getPath()+'/hr/affair/reinstatement/listData'
    }));
	params ={};
...
function uodateBillStatu(ststu){
	 ...
	 if(housePowers.length==0){
		...
		$.post(getPath()+"/hr/affair/reinstatement/updateStatu",{hpStr:hpStr},function(data){
...
function batchApp(){
	...
	art.dialog.confirm("你选择了"+housePowers.length+"个可审批的单据,是否全部通过审批？",function(){
		...
		$(array).each(function(){
			...
			var billId = this.id;
			$.post(ctx+"/hr/affair/reinstatement/approvalBill",{billId:billId},function(data){
				    a++;
					if(data.STATE=="SUCCESS"){
						true_count+=1;
...
function changeStatu(id,billStatu,opName){
	art.dialog.confirm("是否"+opName+"此单据？",function(){
		$.post(ctx+"/hr/affair/reinstatement/changeStatu",{billId:id,billStatu:billStatu},function(data){
    		if(data.STATE=="SUCCESS"){
    			art.dialog.tips(data.MSG);
    			resetList();
...
function approval(id){
	art.dialog.confirm("是否通过审批？",function(){
		$.post(ctx+"/hr/affair/reinstatement/approvalBill",{billId:id},function(data){
    		if(data.STATE=="SUCCESS"){
    			art.dialog.tips(data.MSG);
    			resetList();
    			art.dialog.list["appView"].close();
