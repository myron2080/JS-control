...
$tree_async_url = getPath()+"/hr/person/treeDataAll";
$(document).ready(function(){
	...
	$("#toolBar").append(
		' <div style="padding-top:5px;display:inline;"><span class="greenbtn btn" id="exportBtn"><a href="javascript:void(0)">导出</a></span><label id="includeContainer"><input type="checkbox" name="includeChild" id="includeChild" checked="checked" />包含下级</label></div>'
		+' <div style="padding-top:5px;display:inline;"><label id="disabledContainer">用户状态:<select name="status" id="status"><option value="">全部</option><option value="ENABLE">启用</option><option value="DISABLED">禁用</option></select></div>'
...
function exportPersonByCond(){
	...
	var url = getPath()+"/hr/person/exportPersonByCond?"+param;
