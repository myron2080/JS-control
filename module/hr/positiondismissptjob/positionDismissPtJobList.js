$list_editUrl = getPath()+"/hr/positiondismissptjob/edit";//编辑及查看url
$list_addUrl = getPath()+"/hr/positiondismissptjob/add";//新增及查看url
$list_deleteUrl = getPath()+"/hr/positiondismissptjob/delete";//删除url
$list_dataType = "员工撤职";
$list_editWidth =750;
$list_editHeight = 300;
var manager, g;
...
{display: '操作', name: 'operate', align: 'center', width: 65,render:operateRender},
            ...
            {display: '单据状态', name: 'billStatusName',align: 'left', width: 80,render:statusRender},
            {display: '员工编号', name: 'applyPerson.number', align: 'left', width: 120},
            {display: '员工姓名', name: 'applyPersonName', align: 'left', width: 100},
            {display: '兼职部门', name: 'applyOrg.name', align: 'left', width: 120},
            {display: '兼职职位', name: 'applyPosition.name', align: 'left', width: 120},
            {display: '兼职职级', name: 'applyJoblevel.name', align: 'left', width: 120},
        ...
        delayLoad:true,
        url:getPath()+'/hr/positiondismissptjob/listData'
...
function approveSingle(approveType,billId){
	...
	art.dialog.confirm("是否确认审批所选申请单？",function(){
		$.post(getPath()+"/hr/positiondismissptjob/approve",{billIds:billId,approveType:approveType},function(data){
			if(data.counter > 0){
...
function backApprove(approveType,billId){
	...
	art.dialog.confirm("反审核可能会影响业务数据,是否确认执行反审核？",function(){ 
		$.post(getPath()+"/hr/positiondismissptjob/backApprove",{billId:billId,approveType:approveType},function(data){
...
function approve(approveType){
	...
	art.dialog.confirm("是否确认"+tipTitle+"所选申请单？",function(){
		$.post(getPath()+"/hr/positiondismissptjob/approve",{billIds:billIds,approveType:approveType},function(data){
			if(data.counter > 0){
				art.dialog.data("MSG",null);
				resetList();
			} else {
...
function searchData(){
	...
	$list_dataParam['changeType'] = "DISMISS_PARTTIMEJOB";
	...
	//调动组织
	/*var oldOrgId = $('#orgId').val();
	if(oldOrgId != null && oldOrgId != ""){
		$list_dataParam['oldOrgId'] = oldOrgId;
	}else{
		delete $list_dataParam['oldOrgId'];
	}*/
