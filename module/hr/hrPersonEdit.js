...
function saveEdit(dlg){
	...
	var hightSchool=$("#highestEducation").val();
	if(hightSchool==""){
		art.dialog.tips('最高学历不能为空。');
		return ;
	}
   ...
   var u = $("#agentCertificateTable tbody")
   		.append('<tr name="agentCertificate" index="'+index+'">'
			   +'<td>'
				   ...
				   +'</select>'
			   +'</td>'
			   +'<td><input style="width:60px;" type="text" name="agentCertificateNumber" id="agentCertificateNumber_'+index+'" /></td>'
			   ...
			   +'</td>'
			   +'<td>'
			   		+'<input style="width:60px;"  type="text" name="effectPeriod" id="effectPeriod_'+index+'" />'
			   +'</td>'
			   +'<td>'
			   		+'<input style="width:60px;"  type="text" name="checkDate" id="checkDate_'+index+'" class="Wdate" onfocus="WdatePicker({dateFmt:\'yyyy-MM-dd\'})" />'
			   +'</td>'
			   +'<td><div name="deleteAgentCertificateBtn"><div class="shanchu01"><a></a></div></div></td>'
		   +'</tr>');
   //奖select框的值复制上
	...
	comp.find("tr").each(function(){
	 ...
	 var u = $("#rewardPunishmentTable tbody")
	 		.append('<tr name="rewardPunishmentTr" index="'+index+'">'
	          ...
	          +'</td>'
	          +'<td>'
	          	+'<input style="width:60px;" type="text" id="measure_'+index+'" name="measure"/>'
	          +'</td>'
	          +'<td><div name="deleteRewardPunishmentBtn"><div class="shanchu01"><a></a></div></div></td>'
          +'</tr>');
   //奖select框的值复制上
	...
	comp.find("tr").each(function(){
	 ...
	 var u = $("#educationTable tbody")
	 		.append(' <tr name="education" index="'+index+'">'
		    	...
		    	+'</td>'
		    	+'<td><div name="deleteEducationBtn"><div class="shanchu01"><a></a></div></div></td>'
	    	+'</tr>');
   //奖select框的值复制上
	...
	if($.browser.msie){	//ie
	 ...
	 var u = $("#workExperienceTable tbody")
	 		.append(' <tr name="workExperience"  index="'+index+'">'
	          +'<td align="center"><input style="width:60px;" type="text" name="workStartDate" id="workStartDate_'+index+'"   class="Wdate" onfocus="WdatePicker({dateFmt:\'yyyy-MM-dd\'})"/></td>'
	          ...
	          +'<td align="center"><input style="width:100px;" type="text" onblur="StringUtil.isMobelOrTel(this,true)" name="proverTel" id="proverTel_'+index+'" /></td>'
	          +'<td align="center"><div name="deleteWorkExperienceBtn"><div class="shanchu01"><a></a></div></div></td>'
	          +'</tr>');
...
function addPersonPosition(){
    ...
    var u = $("#addPartTimeJobBtn").parent()
...
//					        +'<td align="left">&nbsp;</td>'
			    ...
			    +'</table>'
		    +'</div>'
		    +'<div style="width:5%; float:left; margin-top:20px;" name=deletePartTimeJobBtn><div class="shanchu01"><a></a></div></div>'
	      +'</div>');
    bindPartTimeJobEvent();
    // ligerui的初始化操作
    $("input,textarea",u).each(function(){
 		$.ligerui.find('Form')[0].options.editorBulider.call(null,$(this));
 	});
}
...
function changeMainPositionOrg(oldValue,newValue,doc){
	...
	if(newValue && ((oldValue == null) || (oldValue.id != newValue.id))){
		...
		var p = $(tr).find('select[name="mainPosition"]');
		p.val(null);
		p.html('');
		var tmpJobLevel = $(tr).find('select[name="mainPositionJobLevel"]');
...
$(document).ready(function(){
	...
	if($edit_viewstate == 'EDIT'){
		/*****************职位****************/
		/*$("select[name='mainPosition']").bind("focus",function(){
			changeMainJob();
		});*/
		changeMainJob();
		...
		/*****************职级****************/
		/*$("select[name='mainPositionJobLevel']").bind("focus",function(){
			initChangeMainJobLevel();
		});*/
		initChangeMainJobLevel();
	...
	var addPersonFlag = $("#addPersonFlag").val();
	if(addPersonFlag!='Y'){
		//输入框
		$('input,textarea,select',$("#mainPositionTbl").get(0)).each(function(){
			$(this).attr('disabled','disabled');
		});
		$('div[class="f7"]',$("#mainPositionTbl").get(0)).each(function(){
			$(this).attr('disabled','disabled');
			
		});
		
		$("input[key='partTimeJobData']").attr('disabled','disabled');
		$("#partTimeJobDataSpan").attr('disabled','disabled');
		$("#addPartTimeJob").attr('disabled','disabled');
		$("div[name='deletePartTimeJobBtn']").attr('disabled','disabled');
	}
});
...
function initChangeMainJobLevel(){
	...
	var jl = $('select[name="mainPositionJobLevel"]');
	var positionId = $('select[name="mainPosition"]').val();
	if(positionId){
		changeJobLevel(jl, positionId,jl.val(),$("#mainPositionJobLevel").val());
	}
}

//改变主要职位的职位的时候
function bindMainPositionEvent(){
	$("select[name='mainPosition']").unbind("change");
	$("select[name='mainPosition']").bind("change",function(){
...
function changeMainJob(){
	...
	changeJob($("select[name='mainPosition']"), $("#mainPositionOrg input[dataPicker='value']").val(),$("#mainPosition").val())
}

//改变职级
function changeJobLevel(selectComp,positionId,jlVal){
	$(selectComp).val(null);
	$(selectComp).html("");
	$.post(getPath()+'/basedata/position/getJobLevelByPosition',{position:positionId},function(res){
		if(res && res.length > 0){
			for(var i = 0; i < res.length; i++){
				if(jlVal && jlVal==res[i].id){
					$('<option value="'+res[i].id+'" selected="selected">'+res[i].name+'</option>').appendTo(selectComp);
				}else{
				 $('<option value="'+res[i].id+'">'+res[i].name+'</option>').appendTo(selectComp);
			    }
			}
		}
	},'json');
...
//改变职位
function changeJob(selectComp,orgId,posVal){
	$(selectComp).val(null);
...
function changeMainJobLevel(obj){
	...
	var jl = topComp.find('select[name="mainPositionJobLevel"]');
	var positionId = topComp.find('select[name="mainPosition"]').val();
	if(positionId){
