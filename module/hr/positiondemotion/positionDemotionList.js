$list_editUrl = getPath()+"/hr/positiondemotion/edit";//编辑及查看url
$list_addUrl = getPath()+"/hr/positiondemotion/add";//新增及查看url
$list_deleteUrl = getPath()+"/hr/positiondemotion/delete";//删除url
$list_dataType = "员工降职";
$list_editWidth =750;
$list_editHeight = 350;
var manager, g;
$(document).ready(function(){
	...
	g =  manager = $list_dataGrid = $("#tableContainer").ligerGrid($.extend($list_defaultGridParam,{
        ...
        columns: [ 
            ...
            {display: '单据编号', name: 'billNumber', align: 'left', width: 127},
            ...
            {display: '降职部门', name: 'applyChangeOrg.name', align: 'left', width: 115},
            {display: '降职职位', name: 'applyChangePosition.name', align: 'left', width: 110},
            {display: '降职职级', name: 'applyChangeJoblevel.name', align: 'left', width: 115},
        ...
        delayLoad:true,
        url:getPath()+'/hr/positiondemotion/listData'
...
function approveSingle(approveType,billId){
	...
	art.dialog.confirm("是否确认"+tipTitle+"所选申请单？",function(){ 
		$.post(getPath()+"/hr/positiondemotion/approve",{billIds:billId,approveType:approveType},function(data){
			if(data.counter > 0){
...
function backApprove(approveType,billId){
	...
	art.dialog.confirm("反审核可能会影响业务数据,是否确认执行反审核？",function(){ 
		$.post(getPath()+"/hr/positiondemotion/backApprove",{billId:billId,approveType:approveType},function(data){
...
function searchData(){
	...
	$list_dataParam['changeType'] = "DEMOTION";
