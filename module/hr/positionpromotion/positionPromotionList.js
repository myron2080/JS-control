$list_editUrl = getPath()+"/hr/positionpromotion/edit";//编辑及查看url
$list_addUrl = getPath()+"/hr/positionpromotion/add";//新增及查看url
$list_deleteUrl = getPath()+"/hr/positionpromotion/delete";//删除url
$list_dataType = "员工晋升";
$list_editWidth =750;
$list_editHeight = 350;
var manager, g;
$(document).ready(function(){
	...
	g =  manager = $list_dataGrid = $("#tableContainer").ligerGrid($.extend($list_defaultGridParam,{
        ...
        columns: [ 
            ...
            {display: '单据编号', name: 'billNumber', align: 'left', width: 130},
            ...
            {display: '变动前部门', name: 'applyOrg.name', align: 'left', width: 115},
            {display: '变动前职位', name: 'applyPosition.name', align: 'left', width: 110},
            {display: '变动前职级', name: 'applyJoblevel.name', align: 'left', width: 115},
            {display: '晋升部门', name: 'applyChangeOrg.name', align: 'left', width: 115},
            {display: '晋升职位', name: 'applyChangePosition.name', align: 'left', width: 110},
            {display: '晋升职级', name: 'applyChangeJoblevel.name', align: 'left', width: 115},
        ...
        delayLoad:true,
        url:getPath()+'/hr/positionpromotion/listData'
...
function approveSingle(approveType,billId){
	...
	art.dialog.confirm("是否确认审批所选申请单？",function(){ 
		$.post(getPath()+"/hr/positionpromotion/approve",{billIds:billId,approveType:approveType},function(data){
			if(data.counter > 0){
...
function backApprove(approveType,billId){
	...
	art.dialog.confirm("反审核可能会影响业务数据,是否确认执行反审核？",function(){ 
		$.post(getPath()+"/hr/positionpromotion/backApprove",{billId:billId,approveType:approveType},function(data){
...
function approve(approveType){
	...
	art.dialog.confirm("是否确认"+tipTitle+"所选申请单？",function(){
		$.post(getPath()+"/hr/positionpromotion/approve",{billIds:billIds,approveType:approveType},function(data){
			if(data.counter > 0){
				resetList();
			} else {
				art.dialog.tips("没有需要审核的数据！！");
...
function searchData(){
	...
	$list_dataParam['changeType'] = "PROMOTION";
