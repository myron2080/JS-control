$list_editUrl = getPath()+"/projectm/pmPhoneMonthReport/edit";//编辑及查看url
...
$(document).ready(function(){
	...
	$list_dataGrid = $("#tableContainer").ligerGrid($.extend($list_defaultGridParam,{
        ...
        height: "98%",
        width:"98%",
        url:getPath()+'/projectm/pmPhoneMonthReport/queryByMonth',
        delayLoad:true,
        usePager:true,
        pageSize:100,
        enabledSort:true
...
function getTatCost(){
	...
	$.post(getPath()+"/projectm/pmPhoneMonthReport/getTotalCost",$list_dataParam,function(data){
		if(data!=null){
...
function viewDeail(data){
	if(typeof(window.top.addTabItem)=='function'){
		window.top.addTabItem( 'detailCall',getPath()+'/projectm/pmPhoneDialDetail/list?infoNumber='+data.infoNumber+'&period='+data.period+'&comboType='+data.comboType,'查询明细');
	}
}
function bindEvent(){
	//查询
