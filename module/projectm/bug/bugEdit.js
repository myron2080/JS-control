...
$(document).ready(function(){
	$("#business").bind('change',function(){
		$('#modula').html('');
		$('#function').html('');
		$.post(getPath()+'/projectm/bug/childModule',{bus:$(this).val()},function(res){
			if(res && res.length > 0){
				for(var i = 0; i < res.length; i++ ){
					$('<option value="'+res[i].id+'">'+res[i].name+'</option>').appendTo($('#modula'));
				}
			}
		},'json');
	});
	...
	$('#modula').bind('change',function(){
		$('#function').html('');
		$.post(getPath()+'/projectm/bug/childMenu',{parent:$(this).val()},function(res){
			if(res && res.length > 0){
				for(var i = 0; i < res.length; i++ ){
...
function setPerson(){
	var functionId = $("#function option:selected").val();	//得到选中的功能id
	$.post(getPath()+'/projectm/bug/getPersonByFunctionId',{functionId:functionId},function(res){
		if(res.person != null ){
			$("#dealPerson input[dataPicker='value']").val(res.person.id);
			$("#dealPerson input[dataPicker='name']").val(res.person.name);
		}
	},'json');
}
...
function initUploader(){
	...
	if(id!=null && id != '' && $edit_viewstate!='VIEW'){
		...
		upload = new SWFUpload({
			upload_url: getPath() + '/projectm/bug/uploadImage',
			file_post_name:'image',
			post_params: {direct:'bug/images',belong:id},
...
//			initUploader();
			...
			if(currentDialog){
				...
				currentDialog.button({name:"确定",disabled:false});
			}
