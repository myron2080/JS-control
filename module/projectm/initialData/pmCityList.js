$list_editUrl = getPath()+"/projectm/cityList/edit";//编辑及查看url
$list_addUrl = getPath()+"/projectm/cityList/add";//新增url
$list_deleteUrl = getPath()+"/projectm/cityList/delete";//删除url
$list_editWidth = "520px";
$list_editHeight = "130px";
$list_dataType = "省份城市";//数据名称
$tree_container = "cityTree";
$tree_async_url = getPath()+"/projectm/cityList/simpleTreeData";
var parentId='';
$(document).ready(function(){
	
	
	$.post(getPath()+'/projectm/cityList/getNode',{}, function(data) { 
	...
	$list_dataGrid = $("#tableContainer").ligerGrid($.extend($list_defaultGridParam,{
        ...
        url:getPath()+'/projectm/cityList/listData',
        delayLoad:true
    }));
...
//新增省份
function addNode(){
	var dlg = art.dialog.open(getPath()+'/projectm/cityList/add', {
		title:"新增",
...
function updateNode(){
	...
	var dlg = art.dialog.open(getPath()+'/projectm/cityList/add?id='+parentId+"", {
		title:"编辑",
		id : 'opernCityAddWindow',
		width :500,
		height :100,
...
function deleteNode(){
	...
	art.dialog.confirm('确定删除该省份?',function(){
		$.post(getPath()+"/projectm/cityList/delete",{id:parentId},function(res){ 
			if(res!=null){
...
function addCity(){
	if(parentId==null ||parentId==""){
		...
		var dlg = art.dialog.open(getPath()+'/projectm/cityList/add?parentId='+parentId, {
			title:"新增",
			id : 'opernCityAddWindow',
			width :500,
			height :120,
...
function updateCity(id){
	...
	var dlg = art.dialog.open(getPath()+'/projectm/cityList/add?id='+id+"", {
		title:"编辑",
		id : 'opernCityAddWindow',
		width :500,
		height :100,
...
function deleteCity(id){
	art.dialog.confirm('确定删除该城市?',function(){
		$.post(getPath()+"/projectm/cityList/delete",{id:id},function(res){ 
			if(res!=null){
				if(res.STATE=='SUCCESS'){
					art.dialog.tips(res.MSG,null,"succeed");
					searchData();
...
//		}
	...
	$.post(getPath()+'/projectm/cityList/listData',$list_dataParam,function(res){ 
