...
function getOrgByTypes(selectObj,orgTypes,selectVal){
	$.post(getPath() + "/basedata/param/autoComplateOrg",{orgTypes:orgTypes},function(data){
		if(data){
			var item = data.items;
			for ( var i = 0; i < item.length; i++) {
...
function validateNumber(obj){
	...
	var _val = $(obj).val();
	if(isNotNull(_val)){
		 $.post(getPath()+"/basedata/param/getParamHeaderByNumber",{number:_val},function(data){
			 var eMsg=$("#errorMsg");
			 if(data.count>0){
				$("#number").val("");
				eMsg.html(_val + " 已被占用");
