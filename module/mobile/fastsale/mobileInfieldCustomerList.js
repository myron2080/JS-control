...
$(document).ready(function(){
	...
	/* timeSeting();
	 $('#showDateDiv a').bind("click",function(){
		 var oType='down';
...
function searchData(str){
	...
	$.post(base+'/mobile/infield/listPagData',param,function(res){
		...
		if(null != res.items&&res.items.length!=0){
			...
			for(var i=0;i<showList.length;i++){
		     ...
		     '</div>');*/
				div +='<div class="yilan-list" objid="'+obj.id+'"  id="bg_'+obj.id+'"';
				div +=        ' onclick=showDetail("'+obj.intentionCustomer.id+'","'+obj.id+'","'+obj.intentionCustomer.person.id+'") ';
				div +=         '>';
			...
			$("#listData").html("");
			var html="";
			html +='<div style="width:95%; min-height:300px; text-align:center; padding-top:30%;">';
...
function showDetail(id,objId,personId){
	window.location.href=base+"/mobilefastsale/intention/view?id="+id+"&objId="+objId+"&personId="+personId;
...
function timeSeting(){
	if($("#queryType").val()=="day"){
		...
		var now=$("#queryDay").attr("key");
		//now="2014-09-27";
		Nowdate=new Date(now.replace(/-/,"/"));
		var tempDate;
		if(Nowdate<showWeekFirstDay()){
		tempDate=Nowdate;
		Nowdate = showPreviousDay() ;
		}
		//Nowdate=new Date();
...
function changeMonth(type) {
	var showMonth = $("#queryMonth").attr("key");
