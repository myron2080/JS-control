...
function searchData(str){
	...
	para.queryOrgLeve=$("table td[class='selected']").attr("key");
	showLoader();
	$.post(base+'/mobile/bi/busiCenter/bsRankDataByOrg',para,function(res){
		...
		if(null != res.items&&res.items.length!=0){
			...
			for(var i=0;i<showList.length;i++){	
				...
				$('<li><span class="Price1">'+showList[i].TOTAL_PRICE+'</span><b class="'+colorIndex+'-ico bold font16 fl">'+accAdd(para.pageSize*(para.currentPage-1),accAdd(1,i))+'</b>'+showList[i].DATANAME+'</li>').appendTo(".Perfolist2 ul");
			}
		}else{
			$(".Perfolist2 ul").html("");
			var html="";
