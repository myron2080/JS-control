...
$(document).on("click", "#moreBtn", function() {
	...
	$.mobile.loading( 'show', {
	 ...
	 var id=$(this).attr("id");
	 viewDtail(id.substring(id.indexOf("_")+1),$(this).attr("key"));
...
function shareTo(){
	...
	var url=url_.replace(/listViewOrg/, "listViewOrgShare")+"?dataCenter="+currdataCenter+"&share=share"+pram;
...
function searchData(str){
	...
	$.post(base+'/weixinapi/mobile/fastsalereport/listPagDataOrg',para,function(res){
		var indnumber = 0;
		if(str == '0'){
			indnumber = 0;
		}else{
		...
		if(null != res.items&&res.items.length!=0){
			...
			for(var i=0;i<showList.length;i++){
				...
				if(obj.NOTLOGINDAYS>=0){
					notLoginDays="("+obj.NOTLOGINDAYS+"天未登录)";
				}
				var CHARGEPERSON_NAME="未设定";
				if(obj.CHARGEPERSON_NAME){
					CHARGEPERSON_NAME = obj.CHARGEPERSON_NAME
				}
			    ...
			    div+="<div class='yilantwo-white' key="+obj.CHARGEPERSON_ID+" id='bg_"+obj.ORG_ID+"'>"
				div+="<div class='xf'>"
				div+="<span class='fl'><b class='"+sortColor+" bold font16 fl'>"+indnumber+"</b>&nbsp;<b class='color333 bold fl'>"+obj.ORG_NAME+"</b></span>   <span class='fr'>经理："+CHARGEPERSON_NAME+notLoginDays+"</span>"
				div+="</div>"
				div+="<div class='xf'>"
...
function changeWeek(type) {
	if (type == 'up'){
		...
		lastDay=new Date(showNextLastWeekDay()).format('yyyy/MM/dd');
		orfirtDay=showWeekFirstDay();
