...
function initUploadImage(button_id){
	var upload = new SWFUpload({
			upload_url: getPath() + '/website/catelog/compressUpload',
...
//				$("#loading_firstPicName").css("display","inline");
			...
			upload_success_handler : function(file, serverData, responseReceived){
				...
				uploadImages[uploadImagesIndex++] = res.PATH;
				fillHtml();
				}else{
...
function fillArr(){
	...
	if(images!="" && images.length>0){
		...
		for(var i = 0 ; i<imgArr.length ; i++){
			if(imgArr[i]!=""){
				uploadImages[uploadImagesIndex++] = imgArr[i];
			}
		}
		fillHtml();
	}
...
function fillHtml(){
	...
	for(i = 0 ; i<uploadImages.length ; i++){
		...
		showPath =  base+'/images/'+(uploadImages[i]?uploadImages[i].replace("size","150X100"):"");
		if(uploadImages[i]!=''){
			resHtml+= '<div>'
				+'<img src="'+showPath+'"/>'
				+'<a href="javascript:iamgeDelete(\''+uploadImages[i]+'\');" style="color: red">删</a> | '
				+'<a href="javascript:imageMove(\''+uploadImages[i]+'\',1);" value="123123" accesskey="565757">前</a> | <a href="javascript:imageMove(\''+uploadImages[i]+'\',-1);">后</a>'
				+'</div>';
		}
	}
...
function imageMove(imgPath , type){
	for(i = 0 ; i<uploadImages.length ; i++){
		if(uploadImages[i]==imgPath){
			if(type==1 && i>0){
				var temp = uploadImages[i-1];
				uploadImages[i-1] = imgPath;
				uploadImages[i] =temp;
				fillHtml();
...
//移除图片  0 1 2 3   ===4
function iamgeDelete(imgPath){
	$.ajax({ url: base+"/website/catelog/deleteImageByPath",type:"post", data:{imgPath:imgPath},dataType:"json", success: function(data){
		for(i = 0 ; i<uploadImages.length ; i++){
			if(uploadImages[i]==imgPath){
				uploadImages[i] = '';
			}
