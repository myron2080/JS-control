...
function disnableData(){
	...
	//输入框
	$('input,textarea,select').not('#confirmDescription,#toStorageName').each(function(){
	...
	if(enter=='Y'){//确认收货操作
		$('#confirmDescription').attr('disabled','disabled');
...
function enterConfirm(){
	$.post(getPath()+'/transferBill/enterConfirm',{
		id:$('#dataId').val(),
		confirmDescription:$('#confirmDescription').val(),
		status:'CONFIRMPAST'
	},function(res){
...
function saveEdit(dlg){
	...
	//$.ajax提交
	var approvalDescription=$('#confirmDescription').val();
...
function noPass(dlg){
	currentDialog = dlg;
	//$.ajax提交
	var approvalDescription=$('#confrimDescription').val();
	if(!approvalDescription || approvalDescription==''){
