...
$(function(){
	...
	$("#startGoldStatus").bind("click", function(){
		var total = parseFloat($("span[key='totalPrice']").text());
...
function selectValue(resultData){
	...
	$.post(base +"/ebhouse/member/getUserInfo",{id:resultData.value},function(data){
		var user = data.user;
		$("#startGold").text(user.startGold + "元");
...
function setDisplayOrHideSettleAccDiv(){
	var totalPrice = parseFloat($("span[key='totalPrice']").html());
	...
	$("#amount").bind("keyup",function(event){  
        ...
        if(!partten.test(numberVal)){       //如果不是数字的话，则将其删除  
        	...
        	if(null != numberVal && "" != $.trim(numberVal)){
        		$("span[key='totalPrice']").html(parseFloat($("td[key='price']").html()*parseInt(numberVal)));
        	} else {
        		$("span[key='totalPrice']").html(0);
        	}
        	setDisplayOrHideSettleAccDiv();
        }  
    });
...
function saveEdit(currentDialog){
	...
	if(compareMoney($("span[key='totalPrice']").html())){
		return;
	}
	...
	$.post(base+"/ebhouse/order/save",{remark:$("#remark").val(),orderJson:getOrderDataJson(),settleAccountJson:settleAccountJson},function(data){
...
function getOrderDataJson(){
	...
	jsonData += "'depositFlag':'" + depositFlagStr + "',";
	...
	jsonData += "'total':'"+$("span[key='totalPrice']").html()+"',";
