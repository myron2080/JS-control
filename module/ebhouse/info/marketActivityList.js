$list_addUrl = getPath()+"/ebhouse/marketactivity/add";//新增url
$list_editUrl = getPath()+"/ebhouse/marketactivity/edit";//编辑及查看url
$list_deleteUrl = getPath()+"/ebhouse/marketactivity/delete";//删除url
$list_previewUrl = getPath()+"/ebhouse/marketactivity/view";//预览url
...
$(document).ready(function() {
		...
		{columns : [ 
					...
					url : getPath()+ '/ebhouse/marketactivity/listData',
					height:'100%',
					delayLoad:true
			}));
		searchlist();
...
function topnotice(id,t){
	$.post(getPath()+"/ebhouse/marketactivity/updatetop",{id:id,istop:t},function(data){
...
function previewRow(id){
	art.dialog.open($list_previewUrl+"?id="+id,
			{
				id : "viewData",
				title : '预览数据',
				width : 750,
...
function enableRow(id,t){
	$.post(getPath()+"/ebhouse/marketactivity/updatetop",{id:id,status:t},function(data){
		if(data.STATE=='SUCCESS'){
			art.dialog.tips(data.MSG);
			resetList();	
		}else{
