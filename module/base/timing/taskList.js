$list_editUrl = getPath()+"/base/task/edit";//编辑及查看url
$list_addUrl = getPath()+"/base/task/add?isValid=1";//新增url
$list_deleteUrl = getPath()+"/base/task/delete";//删除url
...
//		    	   window.top.WorkBench.openPage('taskLog',getPath()+'/base/task/taskLogPage','定时任务日志');
//		       }},
...
//		]
	...
	$("#toolBar").append(
			'<div style="float:left;display:inline;margin-left:5px;">最后执行状态：'+
			'<select id="status" style="width: 90px;">'
			+'<option value="">全部</option>'
			+'<option value="RUNED">执行成功</option>'
	...
	$("#toolBar").append(
		    ...
		    +'		<li class="greenbtn btn"><a href="javascript:void(0)"><span onclick="window.top.WorkBench.openPage(\'taskLog\',\''+getPath()+'/base/task/taskLogPage\',\'定时任务日志\');return false;">任务日志</span></a></li>'
	    	+'	</form>'
	    	+'</div>'
	);
...
//            {display: '当前状态', name: 'status.label', align: 'center', width: 70},
        ...
        url:getPath()+'/base/task/listData'
    }));
...
function lookLog(data){
	window.top.WorkBench.openPage(data.id,getPath()+'/base/task/taskLogPage?taskId='+data.id+'&taskName='+data.name,data.name+"日志");
}

function run(row){
	if(row.status=='WAIT'){
	   art.dialog.confirm("确定立即执行任务?",function(){
		   $.post(getPath()+"/base/task/runTask",{id:row.id},function(res){
...
function setStatus(data){
	var url = getPath()+"/base/task/setStatus";
	if($("#"+data.id).text() != '禁用'){
